...
// Copyright (c) 2015 The sweet developers
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid sweet address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid sweet address:")+s.name_);
...
CScript _createmultisig(const Array& params)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: sweet address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\" (string, required) A json array of keys which are sweet addresses or hex-encoded public keys\n"
            " [\n"
            " \"key\" (string) sweet address or hex-encoded public key\n"
            " ,...\n"
            " ]\n"
            ...
            "sweetd createmultisig 2 \"[\\\"PCHAhUGKiFKDHKW8Pgw3qrp2vMfhwWjuCo\\\",\\\"PJrhyo8CUvFZQT8j67Expre2PYLhavnHXb\\\"]\""
            "\nAs a json rpc call\n"
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // sweet: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; sweet server stopping, restart to run with encrypted wallet";
}
...
// sweet: reserve balance from being staked for network protection
...
// sweet: make a public-private key pair
