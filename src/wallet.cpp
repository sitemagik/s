...
// sweet: optional setting to unlock wallet for block minting only;
...
// sweet: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // sweet: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            for (;;)
            {
                ...
                // sweet: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    if (!GetBoolArg("-avatar")) // sweet: not avatar mode
                    {
...
// sweet: create coin stake transaction
...
// sweet: check 'spent' consistency between wallet and txindex
// sweet: fix wallet spent state according to txindex
...
// sweet: disable transaction (only for coinstake)
