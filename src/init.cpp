...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/sweet.conf are parsed in qt/bitcoin.cpp's main()
        ...
        {
            // First part of help message is specified in sweetd / RPC client
...
{
    ThreadSafeMessageBox(str, _("sweet"), wxOK | wxMODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    ThreadSafeMessageBox(str, _("sweet"), wxOK | wxICON_EXCLAMATION | wxMODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -conf=<file>          "   + _("Specify configuration file (default: sweet.conf)") + "\n" +
    ...
    strUsage += string() +
        _("\nSSL options: (see the sweet Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-reservebalance")) // sweet: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // sweet: checkpoint master priv key
    {
    ...
    // Make sure only a single sweet process is using the data directory.
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  sweet is probably already running."), pszDataDir));
    ...
    if (mapArgs.count("-onlynet")) {
        ...
        BOOST_FOREACH(std::string snet, mapMultiArgs["-onlynet"]) {
            ...
            if (net == NET_UNROUTABLE) {
                ThreadSafeMessageBox(_("Unknown network specified in -onlynet"), _("sweet"), wxOK | wxMODAL);
                return false;
            }
            nets.insert(net);
        }
