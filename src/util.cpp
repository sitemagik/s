...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "sweet.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No sweet.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override sweet.conf
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                if (!fMatch)
                {
                    ...
                    printf("*** %s\n", strMessage.c_str());
                    ThreadSafeMessageBox(strMessage+" ", string("sweet"), wxOK | wxICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "sweet:" << FormatVersion(PEERUNITY_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
